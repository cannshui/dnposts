...
注意，要使上述 `name` 属性的设置有效，编译时必须带上 debug 标记，这样 Spring 才可以从构造器中查找参数名称。如果，你无法在编译你的代码时带上 debug 标记（或不想），你可以使用 [@ConstructorProperties](http://download.oracle.com/javase/6/docs/api/java/beans/ConstructorProperties.html) JDK 注解来显式地为构造器参数设置名称。样例类看起来如下：
...
`static` 工厂方法的参数通过 `<constructor-arg/>` 元素提供，就像构造器中使用的那样。工厂方法返回的对象的类型并不需要是包含 `static` 工厂方法的类的类型，尽管这个例子中是这种情况。一个实例（非静态）工厂方法将会被用于 essentially identical 形式（aside 使用 `factory-bean` 属性替代 `class` 属性），所以细节不会再这里讨论。（译注：ERROR!）

#### 5.4.2 依赖和配置细节

如前面小节中提到的，你可以定义 bean 属性和构造器参数为指向其它被管理的 bean（协作者），或行内定义的值（译注：WHAT?）。为了这个目的，Spring XML 配置方式在 `<property/> 和 `<constructor-arg/>` 元素内部支持子元素类型。
